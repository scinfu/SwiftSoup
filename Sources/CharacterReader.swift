...
public final class CharacterReader {
    ...
    public func consumeToAny(_ chars: Set<UnicodeScalar>) -> String {
        let start = pos
        let utf8Chars = chars.map { Array($0.utf8) }
        ...
        if let result = input.utf8.withContiguousStorageIfAvailable({ buffer -> String in
            ...
            while utf8Pos < buffer.endIndex {
                if utf8Chars.contains(where: { buffer[utf8Pos...].starts(with: $0) }) {
                    pos = input.index(start, offsetBy: utf8Pos - buffer.startIndex)
        ...
        while pos < input.endIndex {
            if chars.contains(input.unicodeScalars[pos]) {
                break
            }
            input.formIndex(after: &pos)
        }
